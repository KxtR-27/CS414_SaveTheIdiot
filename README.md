# VIP Wave Survival
###### Kat R. • Xander C. • Alex B. | CS 414: Game Studio 1

## Useful Links

### Project
- [**Game Design Document**](https://ballstate-my.sharepoint.com/:w:/r/personal/connor_razo_bsu_edu/Documents/Year%204/Semester%201/CS%20414.1%20-%20Game%20Studio%201/Game%20Design%20Document.docx?d=w5e2b538ed03e46b7ac22c865b7de49f0&csf=1&web=1&e=dgnP9t)
- [**Discord Channel**](https://discord.com/channels/829049561867485203/1543041729714982952)

### Individual
- [Kat's Devlogs](https://kxtr-27.github.io/CS414_PersonalDevlogs/)

### Godot docs
- [GDScript Style Guide](docs.godotengine.org/en/stable/tutorials/scripting/gdscript/gdscript_styleguide.html) >
  [Code Order](https://docs.godotengine.org/en/stable/tutorials/scripting/gdscript/gdscript_styleguide.html#code-order)
  
- Godot Best Practices > 
  [Notifications](https://docs.godotengine.org/en/stable/tutorials/best_practices/godot_notifications.html) >
  [`_process()` vs. `_physics_process()` vs. `_input()`](https://docs.godotengine.org/en/stable/tutorials/best_practices/godot_notifications.html#process-vs-physics-process-vs-input)
